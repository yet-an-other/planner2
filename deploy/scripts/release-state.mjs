...
export async function rollbackProduction({
  ...
  const commits = git(cwd, ['log', '--format=%H', '--reverse', '--', relativeValues])
    .split('\n')
    .filter(Boolean)
  const history = []
  ...
  for (const commit of commits) {
    try {
      const text = git(cwd, ['show', `${commit}:${relativeValues}`], false)
      const tag = imageTagLocation(text).tag
      if (tag !== RENDER_ONLY_SEED) history.push(tag)
    } catch {
      // A malformed or absent historical value is not a valid release target.
    }
  }
...
function gitPath(cwd, file) {
  const relative = path.relative(cwd, path.resolve(file)).split(path.sep).join('/')
