...
- Use a 44-point title row and 36-point weekday row beneath the top safe area. _Superseded for builds with the Google connection release gate enabled (development only): the enabled iOS Calendar Header uses a 44-point title/control row with trailing iOS Source Calendar and Account Controls, a fixed 14-point iOS Header Status row, and the 36-point weekday row. The gate stays off in all committed and production configurations, where the dimensions and behavior here remain in force._
...
- The list opens from the Calendar Events model — including Stored Calendar Events presented at process start — with no network call, and it is read-only: the visible cap and the "+N = hidden count" semantics are unchanged, and the popover creates, edits, and deletes nothing.
- The open popover follows refreshed Calendar Events like the Event Detail Popover: a successful Calendar Event Refresh updates list items in place across edits, events moved into the day appear and events moved out of it leave, deleted and declined events leave the list, a failed refresh leaves the list unchanged, and Disconnect on This Device dismisses the popover. The popover dismisses itself when the day's last Calendar Event disappears while it is open.
- Selecting a list item summons that Calendar Event's Event Detail Popover. The two overlays are mutually exclusive — opening either closes the other, so at most one is open at any time, with no stacking and no back navigation; after closing the detail, the day list re-opens via the Events Overflow marker. Mutual exclusivity covers the surface itself: tapping the marker closes any open Event Detail Popover, and summoning the Event Detail Popover from a Calendar Event Bar or Row closes any open Day Events Popover. A drilled-through Event Detail Popover anchors to the drilled-through Date Cell's Events Overflow marker — even when the event also has a visible Calendar Event Bar or Row — because the marker is the anchor that presented the Day Events Popover, and the drill-through is one model mutation, so the two native presentations swap in a single view update on that anchor; a cross-anchor presentation can race the dismissing overlay and be torn down with it. The marker anchors while the event stays attributed to the drilled-through day and the day carries a marker; when a Calendar Event Refresh moves the event off that day or its marker disappears, a visible Calendar Event Bar or Row anchor — or, failing that, another attributed Date Cell's marker — takes over.

### Stored Calendar Events

- Stored Calendar Events (iOS Experience glossary; ADR 0007) are the per-Google-account, device-local copy of the Fetched Window's Calendar Events: the full normalized Calendar Event model exactly as the surface renders it — title, timing, Event Color, location, notes, attendees, and Google link. Raw Google API responses are never persisted.
- The store mirrors the Fetched Window through write-through: every successful initial, slab, or Calendar Event Refresh response updates the store with the in-memory model, so process death never resurrects Calendar Events older than the last successful response. Entries that fall out of the Fetched Window, and Calendar Events from deselected Source Calendars, disappear from the store on the next write; no separate eviction policy exists.
- At process start the surface presents Stored Calendar Events immediately and treats them as always stale: coverage starts empty and the ordinary fetching rules schedule requests as if no events existed. Successful Calendar Event Refresh replaces the stored view atomically; deletions, moves, and declines made elsewhere while offline surface only on that next successful refresh.
- The store lives in Application Support with Data Protection class Complete Until First User Authentication and is excluded from backups; Stored Calendar Events exist only on the device and installation that fetched them.
- Disconnect on This Device wipes the store, and reconnecting refetches: Stored Calendar Events never cross accounts, installations, or devices.
- Storing Calendar Events requires disclosure version 4 acknowledgement according to [`google-account-connection.md`](google-account-connection.md); an installation that has acknowledged only an older disclosure keeps Calendar Events memory-only until it acknowledges the revised explanation.

### Availability and status

- Calendar Events persist as Stored Calendar Events (§ Stored Calendar Events; ADR 0007, superseding ADR 0003): they survive process runs on the fetching device and installation only, and Disconnect on This Device wipes them. There are no Busy-Block-style offline placeholders. Every detail the Event Detail Popover presents shares this boundary.
...
- An offline initial fetch presents Stored Calendar Events when the store holds them — the bare, usable Calendar Grid otherwise — with an offline warning, and retries event-driven on connectivity return; an initial fetch that fails for any other reason reports an error and likewise retains Stored Calendar Events. A failed slab keeps already-fetched events visible with a fetch-issue message and recovers on connectivity return or the next edge approach.
...
- Persistence, restoration state, networking, permission, analytics, user notification, or extension. _Superseded only as needed by the gated Google Account Connection, Selected Source Calendars, and Calendar Events: enabled builds persist disclosure and installation markers plus account-keyed Selected Source Calendar IDs (ADR 0006), reach Google for authorization, profile presentation, Source Calendars, and Calendar Events, request Calendar read authorization, and persist Stored Calendar Events in backup-excluded Application Support storage wiped by Disconnect on This Device (ADR 0007); no other persistence, networking, permission, analytics, notification, or extension exists, and the gate keeps the additions inactive in committed and production builds._
...
| Day Events Popover | Deterministic SwiftUI previews plus observable Calendar Events model coverage (gate on) | Automated pass: summoning lists the Date Cell's complete ordered day — visible and cap-hidden items alike, bars in lane order including multiday crossings, then rows by start time — from memory with no network call, reconciling live across successful refresh (edit/move in place, deletion/decline removal, failure retention), dismissing on Disconnect on This Device and when the day empties; drill-through summons the Event Detail Popover with mutual exclusivity in both directions, re-open after close, and drilled-through marker anchoring; hosted-presentation coverage drives the real Calendar Screen through drill-throughs for visible and cap-hidden events, while tap, dismissal, and sheet-adaptation runs remain pending |
...
| Stored Calendar Events | Gate-on simulator: fetch, force-quit, relaunch online and offline, disconnect | Pending — stored events present immediately at relaunch, online refresh replaces them, offline relaunch retains them with a warning, Disconnect on This Device wipes the store |
