...
- Selecting a list item summons that Calendar Event's Event Detail Popover. The two overlays are mutually exclusive — opening either closes the other, so at most one is open at any time, with no stacking and no back navigation; after closing the detail, the day list re-opens via the Events Overflow marker. Mutual exclusivity covers the surface itself: tapping the marker closes any open Event Detail Popover, and summoning the Event Detail Popover from a Calendar Event Bar or Row closes any open Day Events Popover. When the drilled event has no visible Calendar Event Bar or Row, the Event Detail Popover anchors to the drilled-through Date Cell's Events Overflow marker while the event stays attributed to it; when a Calendar Event Refresh moves the event off that day, another attributed Date Cell's marker takes over.
