...
- Google Account Connection or Source Calendar. _Superseded only for development builds with the Google connection release gate enabled, which present the gated iOS Account Control and iOS Header Status with launch restoration, the first-connect explanation, Connect, and Disconnect on This Device; connectivity-aware offline recovery and the installation boundary remain excluded here and arrive in later slices._
