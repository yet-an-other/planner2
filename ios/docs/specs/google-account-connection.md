...
- Every form carries the connection dot: a badge floating six points past the capsule's top-trailing corner that presents the Google Account Connection state at a glance — green when connected, gray when disconnected, pulsing gray while restoring or connecting. Connection warnings and errors never change the dot; the iOS Header Status text carries them. The dot adds no width to the trailing control cluster, mirrors naturally for right-to-left, and is folded into the control's own accessibility rather than exposed as a separate element.
