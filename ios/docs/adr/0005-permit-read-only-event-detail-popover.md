...
4. All detail it presents was originally **memory-only while connected**, extending iOS ADR 0003's boundary from titles and timing to all event detail. **Superseded by ADR 0007:** normalized detail, including Calendar Event Attachments, now persists inside Stored Calendar Events; Disconnect on This Device wipes that store and clears the open popover.
...
- The Google Calendar seam grows to decode event detail (Google link, location, notes, attendees) and to retain timed events' end instants for the timing line. It originally stayed memory-only per iOS ADR 0003; ADR 0007 now persists the normalized detail, including Calendar Event Attachments, as Stored Calendar Events.
...
- Event detail originally added no persistence and remained covered by the Calendar Event disclosure boundary. ADR 0007 supersedes that consequence by persisting full normalized detail as Stored Calendar Events; Source Calendar selection separately stores selected IDs under ADR 0006.
