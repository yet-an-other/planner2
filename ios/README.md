...
A configured development build runs one Connect flow for Google identity and read-only Calendar access through the official Google Sign-In SDK, shows the compact connected control with its Disconnect on This Device affordance, and disconnects through local SDK sign-out only — never SDK disconnect or Google revocation, so sibling Planner connections stay intact. Planner persists no tokens or account profile data itself; Google Sign-In owns credential storage. Restoration across launches, the first-connect explanation, offline recovery, and the installation boundary arrive in later slices.
